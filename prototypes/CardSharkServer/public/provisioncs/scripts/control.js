...
var serverURL = 'http://cardsharksystems.com'
...
$(document).ready(function() {
	...
	$('#loginSubmit').click(function(event) {
		$.getJSON(serverURL + '/provisioncs/Login' + '?Callback=?',
		{
	...
	$('#addUser').click(function(event) {
		...
		$('#submitEdit').click(function(event) {
			$.getJSON(serverURL + '/provisioncs/AddUser' + '?Callback=?',
			{
				userID: userID
			},
			function(data, status, jqXHR) {
	...
	$('#codeSubmit').click(function(event) {
		if($('input[name="time"]:checked').length < 1) {
			alert('select something first')
		}
		else {
			$.getJSON(serverURL + '/provisioncs/SubmitCode' + '?Callback=?',
			{
				userID:		userID,
				days:		$('input[name="time"]:checked').val(),
...
function loadTable() {
	$.getJSON(serverURL + '/provisioncs/GetUsers' + '?Callback=?',
	{
		userID: userID
	},
	function(data, status, jqXHR) {
...
function editUser(scope, event, user) {
	...
	$('#submitEdit').click(function(event) {
		...
		$.getJSON(serverURL + '/provisioncs/UpdateUser' + '?Callback=?',
		{
			userID			:userID,
...
/*
$.getJSON(serverURL + '/provision/INSERTMESSAGEHERE' + '?Callback=?',
{
	userID: userID
},
function(data, status, jqXHR) {
